			 ...
			 struct aptina_pll *pll)
{
	...
	p1_min = max(limits->p1_min, DIV_ROUND_UP(limits->out_clock_min * div,
		     pll->ext_clock * pll->m));
	dev_dbg(dev, "PLL: p1_min %u\n", p1_min);
	p1_max = min(limits->p1_max, limits->out_clock_max * div /
		     (pll->ext_clock * pll->m));
	dev_dbg(dev, "PLL: p1_max %u\n", p1_max);

	for (p1 = p1_max & ~1; p1 >= p1_min; p1 -= 2) {
		dev_dbg(dev, "PLL: p1 %u\n", p1);
		unsigned int mf_inc = p1 / gcd(div, p1);
		dev_dbg(dev, "PLL: mf_inc %u\n", mf_inc);
		unsigned int mf_high;
		unsigned int mf_low;

		mf_low = roundup(max(mf_min, DIV_ROUND_UP(pll->ext_clock * p1,
					limits->int_clock_max * div)), mf_inc);
		dev_dbg(dev, "PLL: mf_low %u\n", mf_low);
		mf_high = min(mf_max, pll->ext_clock * p1 /
			      (limits->int_clock_min * div));

		dev_dbg(dev, "PLL: mf_high %u\n", mf_high);
		if (mf_low > mf_high)
			continue;
